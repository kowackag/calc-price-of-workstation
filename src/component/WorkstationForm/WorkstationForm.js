...
import StyledWorkstationForm, {
    Container
} from './WorkstationForm.styled.js';
...
const WorkstationForm = () => {
        ...
        useEffect(() => {
            const copyList = category ?
                productsList.filter(item => item.category === category).map(({
                    type
                }) => type) :
                productsList.map(({
                    type
        ...
        useEffect(() => {
            const copyList = type ?
                productsList.filter(item => item.type === type).map(({
                    model
                }) => model) :
                productsList.map(({
                    model
        ...
        const setValue = e => {
            ...
            setState({
                ...state,
                [e.target.dataset.name]: e.target.dataset.code
            })
        }
        ...
        return ( <
            StyledWorkstationForm onSubmit = {
                handleSubmit
            } >
            <
